<?php
namespace App\Foundation\Http\Websocket;
...
enum Opcode: int
{
    ...
    {
        return $this->value >= self::CLOSE->value;
    }
    ...
    {
        return $this->value <= self::BINARY->value;
    }
...
class WebSocketServer
{
    ...
    {
        echo "PING received from {$clientId}\n";

        // Respond with PONG
        $this->sendFrame($socket, $frame->payload, Opcode::PONG);
    ...
    {
        // echo "PONG received from {$clientId}\n";
     ...
     */
    private function tryParseFrame(string $data): ?WebSocketFrame
    {
        if (strlen($data) < 2) {
            return null;
        }
     ...
     */
    private function processFrame(string $clientId, WebSocketFrame $frame, $socket): void
    {
        // Update frame count
        if (!isset($this->clients[$clientId]['frame_count'])) {
            $this->clients[$clientId]['frame_count'] = 0;
        }
        $this->clients[$clientId]['frame_count']++;
        ...
        // Handle data frames
        if (isset($this->eventHandlers['data'])) {
            foreach ($this->eventHandlers['data'] as $handler) {
                try {
                    $handler($clientId, $frame, $socket);
                } catch (Throwable $e) {
                    echo "Error in data handler for {$clientId}: " . $e->getMessage() . "\n";
                    $this->emit('error', [
                        'clientId' => $clientId,
                        'error' => $e->getMessage(),
                        'timestamp' => time()
    ...
    /**
     * Process client buffer to extract complete WebSocket frames
     */
    private function processClientBuffer(string $clientId, $socket): void
    {
        if (!isset($this->clients[$clientId]['buffer'])) {
            return;
        }
        ...
        // Keep processing while we have enough data for at least a frame header
        while ($bufferLength >= 2 && $offset < $bufferLength) {
            $firstByte = ord($buffer[$offset]);
            $secondByte = ord($buffer[$offset + 1]);

            // Get initial payload length
            $payloadLength = $secondByte & 0x7F;
            $frameStart = $offset;

            // Calculate frame header size
            $headerSize = 2; // Base header size

            // Extended payload length
            if ($payloadLength === 126) {
                if ($bufferLength < $offset + 4) {
                    break; // Not enough data for 16-bit length
                }
                $headerSize += 2;
            } elseif ($payloadLength === 127) {
                if ($bufferLength < $offset + 10) {
                    break; // Not enough data for 64-bit length
                }
                $headerSize += 8;
            }

            // Check if we have masking key
            $masked = ($secondByte & 0x80) !== 0;
            if ($masked) {
                $headerSize += 4;
            }
            ...
            $totalFrameSize = $headerSize + $payloadLength;
            if ($bufferLength - $offset < $totalFrameSize) {
                break; // Incomplete frame, wait for more data
            }

            // Extract complete frame
            $frameData = substr($buffer, $offset, $totalFrameSize);

            // Parse the frame
            $frame = $this->tryParseFrame($frameData);
            
            if ($frame === null) {
                echo "Invalid frame received from {$clientId}\n";
                $this->sendClose($clientId, CloseCode::PROTOCOL_ERROR, 'Unmasked client fram');
                $this->removeClient($socket);
                return;
            }

            if ($frame->rsv1 || $frame->rsv2 || $frame->rsv3) {
                $this->sendClose($clientId, CloseCode::PROTOCOL_ERROR, 'RSV bits set');
                $this->removeClient($socket);
                return;
            }
            ...
            if (!$frame->fin) {
                // This is a fragmented frame - store it and wait for continuation
                if (!isset($this->clients[$clientId]['fragments'])) {
                    $this->clients[$clientId]['fragments'] = [
                        'buffer' => '',
                        'opcode' => $frame->opcode,
                        'total_length' => 0
                    ];
                ...
                $this->clients[$clientId]['fragments']['buffer'] .= $frame->payload;
                $this->clients[$clientId]['fragments']['total_length'] += $frame->payloadLength;

                // Check if message exceeds maximum size
                if ($this->clients[$clientId]['fragments']['total_length'] > $this->maxMessageSize) {
                    echo "Fragmented message too large from {$clientId}\n";
                    $this->sendClose($clientId, CloseCode::MESSAGE_TOO_BIG);
                    unset($this->clients[$clientId]['fragments']);
                    $this->removeClient($socket);
                    return;
                }

                // Move offset and continue
                $offset += $totalFrameSize;
                continue;
            } elseif (isset($this->clients[$clientId]['fragments'])) {
                // This is the final frame of a fragmented message
                $fragments = $this->clients[$clientId]['fragments'];

                // Combine all fragments
                $completePayload = $fragments['buffer'] . $frame->payload;
                $completeLength = $fragments['total_length'] + $frame->payloadLength;

                // Create a complete frame from fragments
                $completeFrame = new WebSocketFrame(
                    true, // fin
                    $frame->rsv1,
                    $frame->rsv2,
                    $frame->rsv3,
                    $fragments['opcode'], // Use original opcode
                    $frame->masked,
                    $completeLength,
                    null, // No masking key for combined frame
                    $completePayload
                );

                // Process the complete frame
                $this->processFrame($clientId, $completeFrame, $socket);

                // Clean up fragments
                unset($this->clients[$clientId]['fragments']);
            } else {
                // Single, complete frame
                $this->processFrame($clientId, $frame, $socket);
            }
        ...
        if ($offset > 0) {
            $this->clients[$clientId]['buffer'] = ($offset < $bufferLength)
                ? substr($buffer, $offset)
                : '';

            // Trim buffer if it gets too large (prevent memory exhaustion attack)
            if (strlen($this->clients[$clientId]['buffer']) > $this->maxMessageSize * 2) {
                echo "Client {$clientId} buffer too large, closing connection\n";
                $this->sendClose($clientId, CloseCode::MESSAGE_TOO_BIG);
                $this->removeClient($socket);
            }
        }
     ...
     *
     * @param string $event
     * @param callable(string $client, WebSocketFrame $frame, resource|\Socket $socket)|Closure(string $client, WebSocketFrame $frame, resource|\Socket $socket) $callback
     * @return void
     */
    public function on(string $event, callable|Closure $callback): void
    {
    ...
    private function sendPeriodicPings(): void
    {
        ...
        foreach ($this->clients as $clientId => $client) {
            ...
            $lastPingSent = $client['last_ping_sent'] ?? 0;
            $lastPongReceived = $client['last_pong'] ?? 0;

            // Check if enough time has passed since last ping
            $timeSinceLastPing = $now - $lastPingSent;
            $timeSinceLastPong = $now - $lastPongReceived;

            // Only send ping if:
            // 1. No ping sent yet OR enough time has passed since last ping
            // 2. AND no recent activity from client (they haven't sent us data)
            // 3. AND we're not waiting for a pong response (unless timeout)
            $pingBackoffTime = max(10, $this->pingInterval / 2); // Minimum 10 seconds between pings

            if ($lastPingSent === 0 || $timeSinceLastPing >= $pingBackoffTime) {
                if ($now - $lastActivity >= $this->pingInterval) {
                    // Check if we're waiting for a pong (ping sent but no response)
                    if ($lastPingSent > 0 && $lastPingSent > $lastPongReceived) {
                        // We sent a ping but haven't received pong - check if timeout
                        $pingTimeout = 30; // seconds to wait for pong
                        if ($now - $lastPingSent >= $pingTimeout) {
                            // Ping timeout - client is unresponsive
                            echo "Ping timeout for client {$clientId}, closing connection\n";
                            $this->sendClose($clientId, CloseCode::GOING_AWAY, 'Ping timeout');
                    ...
                    // Send ping to inactive client
                    $this->sendPing($clientId, 'ping_' . time());
                    echo "Sent ping to inactive client: {$clientId}\n";
                }
            }
        }
    }
    ...
    private function acceptConnection()
    {
        ...
        if ($clientSocket !== false) {
            ...
            // Check request protocol (Only accept GET)
            ...
            // Check for WebSocket Key, if not present abort
    ...
    private function removeClient($socket)
    {
        ...
        if ($clientId !== null) {
            ...
            if (isset($this->clients[$clientId]['buffer'])) {
                unset($this->clients[$clientId]['buffer']);
            }
