...
class ClassManager
{
    ...
    public static function autoload($class)
    {
        try {
            ...
            if (self::$auto_resolve && $e->getCode() == 6) {
                self::method_7($class)
                    ? self::messageForResolvedClass($class, 7)
                    :  throw new \Exception($class . '::class has not yet registered. And cannot be resolved.', 404, $e);
            } elseif (self::$auto_resolve) {
                self::method_2($class)
    ...
    /**
     * Method 2 searching algorithm (A.K.A. Direct look up to most common place)
     * ___
     * 
    ...
    /**
     * Method 3 searching algorithm (A.K.A. Look up to cached path)
     * ___
     * 
     * Hierarchy of class searching Algorithm and Big O notation speed
     *
    ...
    {
        if (self::$debug){ error_log('Auto-loader: Using method 5');}
        ...
        if (self::$debug) {error_log('Auto-loader: Method 5 failed (skipped)');}
        return false;
    }
    ...
    {
        if (self::$debug){ error_log('Auto-loader: Using method 6');}
        if (self::$is_web && getBoolEnv('AUTO_LOAD_USER_PATH_DEFINED_CLASS', self::$auto_resolve)) {
            return showErrorPage(HTTP_SERVER_ERROR, 'Add new class: ' . $class, 'Register ' . $class . '\'s path', '', !empty($e) ? $e->getTraceAsString() : null, $e, true);
        }
        if (self::$debug) {error_log('Auto-loader: Method 6 failed (skipped)');}
        return false;
    }

    /**
     * Method 7 searching algorithm (A.K.A. Creating placeholder class, except setting config to false in `.env`)
     * ___
     * 
     * Hierarchy of class searching Algorithm and Big O notation speed
     *
    ...
    public static function method_7($class)
    {
        if (getBoolEnv('AUTO_LOAD_CLASS_ALWAYS_EXISTS', self::$auto_resolve)) {
            if (self::$debug) {error_log('Auto-loader: Using method 7');}
            eval("class $class{
        ...
        if (self::$debug) {error_log('Auto-loader: Method 7 failed (skipped)');}
        return false;
    }
}
